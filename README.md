...
distribution plot from samples
    ...
    usage: dist [-ajfgx] [-b BINS] [-s SMOOTH] [-m MARKFILE] SAMFILE OUTFILE
...
The `-ajfg` flags control the plotting of anchor points (`-a`), magnification
ratios (`-j`), convergence ratios (`-f`) and reduced shears (`-g`),
respectively, and at least one of these flags must be given.

The `-x` flag enables printing of the central value and confidence bounds for
each plot. Values are obtained from the 50th, 16th and 84th percentile.
...
The `-m` option reads values from `MARKFILE` and marks them in the plot. The
precise layout (rows and columns) of the file does not matter, as long as the
number and order of the values agrees with the plot.
